...
const renderCenter = (centerLabel, centerFontSize, color, currentPage) => (
  <View style={{width: Dimensions.get('window').width * 0.50}}>
    {
      typeof centerLabel === 'object' ? centerLabel :
        <Text style={[styles.centerLabel, { color }, {width: Dimensions.get('window').width * 0.50, textAlign: 'center'}, {fontSize: centerFontSize}]}>
          ...
          {
            currentPage !== 0 && (
              <Text style={styles.centerColorLabel}>{currentPage}</Text>
            )
          }
          {
            currentPage !== 0 && '/9'
...
export const TopNavBar = ({
  ...
  if (leftIcon === true) {
    iconLeft = <Icon name="ios-arrow-back" size={30} style={[styles.iconLeft, { color: leftIconColor }]} />;
  } else if (leftIcon !== false) {
    iconLeft = <Icon name={leftIcon} size={20} style={[{}, { color: leftIconColor }]} />;
  }  
  if (rightIcon === true) {
  ...
  return (
    <View style={[styles.container, { backgroundColor }, styles.borderBottom, {borderBottomColor: bottomColor}, wrapStyle]}>
      {leftLabel ?
        renderLeftSide(iconLeft, leftLabel, leftAction, sideFontSize, color, sideWidth) :
        <View style={styles.button} />
      }
      {centerLabel ? renderCenter(centerLabel, centerFontSize, color, currentPage) : <View /> }
